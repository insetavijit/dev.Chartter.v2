...
if __name__ == "__main__":
    print("=== Enhanced Technical Analysis Framework - Refactored Version ===\n")
    ...
    analyzer = create_analyzer(sample_data, config)

    # Demo 1: Enhanced query execution
    print("Demo 1: Enhanced Query Execution")
    query = """
    Close above EMA_21
    RSI_14 below 70
    ...
    # Demo 2: Performance report with cache statistics
    print("Demo 2: Enhanced Performance Report")
    perf_report = analyzer.performance_report()
    print("Summary:", perf_report['summary'])
    print("Cache Stats:", perf_report['cache_statistics'])
    print("Memory Usage:", perf_report['memory_usage'])
    print()

    # Demo 3: Advanced signal generation
    print("Demo 3: Advanced Signal Generation")
    analyzer.generate_trend_following_signals()

    # Demo 4: Enhanced backtesting
    signal_columns = [col for col in analyzer.df.columns if 'signal' in col.lower()]
    if signal_columns:
        backtest_results = analyzer.backtest_signals(signal_columns[0], holding_period=5)
        print("Enhanced Backtest Results:")
        for key, value in backtest_results.items():
            print(f"  {key}: {value}")

    print(f"\nFinal DataFrame shape: {analyzer.df.shape}")
    print(f"Available indicators: {len(analyzer.indicator_engine._available_indicators)}")
    print("Framework successfully refactored with clean architecture!")
